...
![](https://images.pexels.com/photos/1148820/pexels-photo-1148820.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=175)
...
```{r}
tibble("Row"=seq(3,10,1)) %>% mutate(Zeilen = 10^Row, Dauer_pro_Zeile=.001,Knoten=8,Maps=Knoten,Reduces=Knoten,Overhead=.1*(Maps+Reduces),Dauer_DB=Zeilen*Dauer_pro_Zeile,Dauer_MR=(Zeilen/Knoten)*Dauer_pro_Zeile+Dauer_pro_Zeile*Maps+Dauer_pro_Zeile*Reduces+Overhead,Zeilen=format(Zeilen,scientific = F,big.mark = ".",decimal.mark = ","),Unterschied=paste(round(100*(Dauer_MR/Dauer_DB)),"%"),Dauer_MR=round(Dauer_MR),Dauer_DB=round(Dauer_DB)) %>% select(-Row,-Maps,-Reduces,Overhead) %>% knitr::kable()
```
...
Die Analyse dauert mit `r paste(dauer,"ms")` zwar länger als im RAM (~150 ms), aber nur etwa halb so lange wie mit SQLite (~700 ms). 
